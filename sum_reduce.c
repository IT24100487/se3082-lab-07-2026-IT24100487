...
int main(int argc, char **argv) {
    ...
    /* Reduce combines every local_sum with MPI_SUM and lands the result on root
       in a single O(log P) tree-based call — no gather buffer, no manual loop. */
    long long total_sum = 0;
    MPI_Reduce(&local_sum, &total_sum, 1, MPI_LONG_LONG, MPI_SUM, 0, MPI_COMM_WORLD);

    if (rank == 0) {
        /* total_sum is only valid here on root. */
